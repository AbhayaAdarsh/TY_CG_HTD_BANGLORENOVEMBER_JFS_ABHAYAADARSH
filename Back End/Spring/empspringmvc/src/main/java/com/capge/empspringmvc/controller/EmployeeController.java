...
public class EmployeeController {
	...
	public String login(String email,String password,ModelMap map,HttpServletRequest request) {
		...
		session.setAttribute("bean", bean);//I will get whole bean
		return "home";
	}
		@GetMapping("/register")
		...
		public String register(EmployeeBean bean,ModelMap map)
		{
			...
			if(check) {
				...
				map.addAttribute("msg","The name is already exist!!!.");
			}
			return "login";
		}
		...
		@GetMapping("/changepassword")//for go to the form
		public String changePassword(@SessionAttribute(name="bean",required=false) EmployeeBean bean) {
			if(bean!=null)//for validation
				return "changepassword";
			else 
				return "login";
		}
		
		@PostMapping("/changepassword")//make changes to the form
