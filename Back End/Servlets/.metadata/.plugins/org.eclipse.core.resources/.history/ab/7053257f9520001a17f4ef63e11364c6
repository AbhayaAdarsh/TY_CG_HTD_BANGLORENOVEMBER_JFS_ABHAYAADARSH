...
private EmployeeServices service= new EmployeeServiceImpl();
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		HttpSession session=req.getSession(false);//to give null if user is not valid so we write false so that it will go to same session ,true create new session
